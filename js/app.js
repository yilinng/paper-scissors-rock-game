...
function printItem(item, youPickValue){
		...
		//display target
		allItems.classList.add('grid', 'grid-rows-2','grid-flow-col' ,'gap-8');
		item.classList.remove('h-32', 'w-32','z-10','mt-40' ,'ml-20','-mt-12' ,'ml-48','-mt-12' ,'-ml-4');
		item.classList.add('z-40','relative','h-48', 'w-48', 'youPick','-mt-20','ml-10');
	...
	let itemTextr = document.createElement('span');
		itemTextr.setAttribute('class','z-40 font-bold text-white text-2xl thehouse')	
	...
	let itemTextl = document.createElement('span');
		itemTextl.setAttribute('class','z-40 font-bold text-white text-2xl ml-20 youPicked')	
	...
	let housePick = document.createElement('div');
	   housePick.setAttribute('class','rounded-full h-48 w-48 flex items-center justify-center bg-gray-800 -mt-20 mr-16 housePick');
	//https://www.w3schools.com/JSREF/met_node_insertbefore.asp
...
function houseChoose(housePicks){
	...
	let houseChooses = document.createElement('img');
		...
		housePicks.classList.add(randomItem.color,'bg-white','border-8','shadow-outline','z-40');			
}
...
function ruleToOutcome(youPickValue, housePicks){
	...
	function winTurn(){
		...
		allItems.classList.remove('grid-rows-2','-ml-24');
		allItems.classList.add('grid-cols-3','grid-rows-2');

		//console.log(allItems.childNodes);

		let centerText = document.createElement('div');
		centerText.setAttribute('class','z-40 place-self-center font-bold text-white text-3xl centerPick')	
		...
		centerBtn.setAttribute('class','z-40 place-self-center bg-white text-3xl text-pink-400 rounded p-2 -mt-20 playAgain');
	...
	function loseTurn(){
		...
		circle2.setAttribute('style','hsl(214, 47%, 23%) animation-delay: 1s');

		let circle3 = document.createElement('div');
		circle3.setAttribute('class','z-10 animate-pulse absolute transform scale-150 rounded-full h-64 w-64  bg-opacity-100 shadow-2xl');
		circle3.setAttribute('style','hsl(214, 47%, 23%) animation-delay: 2s');
	...
	function tryAgain(){
		...
		let centerBtn = document.createElement('button');
		centerBtn.setAttribute('class','place-self-center bg-white text-3xl text-pink-400 rounded p-2 -mt-20 playAgain');
		centerBtn.innerText = 'Play again!';
   ...
   function displayGame() {
    ...
    <div class="z-10 absolute -mt-12 -ml-4 rounded-full h-32 w-32 flex items-center justify-center bg-white border-8 border-blue-500 shadow-outline cursor-pointer paper">
...
<div class="z-10 absolute  mt-40 ml-20 rounded-full h-32 w-32 flex items-center justify-center bg-white border-8 border-red-500 shadow-outline cursor-pointer rock">
...
<div class="z-10 absolute -mt-12 ml-48 rounded-full h-32 w-32 flex items-center justify-center bg-white border-8 border-yellow-500 shadow-outline cursor-pointer scissors">

      <input type="hidden" value="scissors">
